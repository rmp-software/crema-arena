...
- **Feature specs** live in `docs/specs/<slug>.md`, driven by the `spec-feature` → `breakdown-feature` → `work-iteration` skills (frontmatter carries the Linear parent issue + feature branch). `scripts/repair-bracket-cascade.ts` backfills `next_duel_id` / `next_duel_slot` on bracket rows that predate the cascade fix.
